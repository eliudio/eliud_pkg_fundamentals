...
class BookletForm extends StatelessWidget {
  ...
  BookletForm({Key? key, required this.formAction, required this.value, this.submitAction}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    if (formAction == FormAction.ShowData) {
      return BlocProvider<BookletFormBloc >(
        ...
        child: MyBookletForm(submitAction: submitAction, formAction: formAction),
          );
      ...
      return Scaffold(
        appBar: StyleRegistry.registry().styleWithContext(context).adminFormStyle().constructAppBar(context, formAction == FormAction.UpdateAction ? 'Update Booklet' : 'Add Booklet'),
...
class MyBookletForm extends StatefulWidget {
  ...
  MyBookletForm({this.formAction, this.submitAction});
    ...
    return BlocBuilder<BookletFormBloc, BookletFormState>(builder: (context, state) {
      ...
      if (state is BookletFormInitialized) {
        ...
        children.add(
                  ...
                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'Document ID', Icons.vpn_key, (formAction == FormAction.UpdateAction), _documentIDController, FieldType.String, validator: (_) => state is DocumentIDBookletFormError ? state.message : null, hintText: 'null')
          );

        children.add(

                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'Name', Icons.text_format, _readOnly(accessState, state), _nameController, FieldType.String, validator: (_) => state is NameBookletFormError ? state.message : null, hintText: 'null')
          );
        ...
        children.add(StyleRegistry.registry().styleWithContext(context).adminFormStyle().divider(context));
         ...
         children.add(Container(
                  alignment: Alignment.centerLeft,
                  padding: const EdgeInsets.fromLTRB(0, 20, 0, 20),
                  child: StyleRegistry.registry().styleWithContext(context).adminFormStyle().groupTitle(context, 'Font Title')
                ));
        ...
        children.add(StyleRegistry.registry().styleWithContext(context).adminFormStyle().divider(context));
         ...
         children.add(Container(
                  alignment: Alignment.centerLeft,
                  padding: const EdgeInsets.fromLTRB(0, 20, 0, 20),
                  child: StyleRegistry.registry().styleWithContext(context).adminFormStyle().groupTitle(context, 'Font Text')
                ));
        ...
        children.add(StyleRegistry.registry().styleWithContext(context).adminFormStyle().divider(context));
         ...
         children.add(Container(
                  alignment: Alignment.centerLeft,
                  padding: const EdgeInsets.fromLTRB(0, 20, 0, 20),
                  child: StyleRegistry.registry().styleWithContext(context).adminFormStyle().groupTitle(context, 'Sections')
                ));
