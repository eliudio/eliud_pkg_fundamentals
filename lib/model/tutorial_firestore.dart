...
class TutorialFirestore implements TutorialRepository {
  ...
  Future<TutorialModel?> get(String? id, {Function(Exception)? onError}) {
    return TutorialCollection.doc(id).get().then((doc) async {
      if (doc.data() != null)
...
//    In the meantime:
      stream = getQuery(appRepository()!.getSubCollection(appId, 'tutorial'), orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?,  limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots().map((data) {
      Iterable<TutorialModel?> tutorials  = data.docs.map((doc) {
        TutorialModel? value = _populateDoc(doc);
        return value;
      }).toList();
      return tutorials as List<TutorialModel?>;
    });
...
//  see comment listen(...) above
    stream = getQuery(appRepository()!.getSubCollection(appId, 'tutorial'), orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?,  limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots()
        .asyncMap((data) async {
  ...
  Stream<List<TutorialModel?>> values({String? orderBy, bool? descending, Object? startAfter, int? limit, SetLastDoc? setLastDoc, int? privilegeLevel, EliudQuery? eliudQuery }) {
    ...
    Stream<List<TutorialModel?>> _values = getQuery(TutorialCollection, orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?, limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots().map((snapshot) {
