...
class DividerFirestore implements DividerRepository {
  ...
  Future<DividerModel?> get(String? id, {Function(Exception)? onError}) {
    return DividerCollection.doc(id).get().then((doc) async {
      if (doc.data() != null)
...
//    In the meantime:
      stream = getQuery(appRepository()!.getSubCollection(appId, 'divider'), orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?,  limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots().map((data) {
      Iterable<DividerModel?> dividers  = data.docs.map((doc) {
        DividerModel? value = _populateDoc(doc);
        return value;
      }).toList();
      return dividers as List<DividerModel?>;
    });
...
//  see comment listen(...) above
    stream = getQuery(appRepository()!.getSubCollection(appId, 'divider'), orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?,  limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots()
        .asyncMap((data) async {
  ...
  Stream<List<DividerModel?>> values({String? orderBy, bool? descending, Object? startAfter, int? limit, SetLastDoc? setLastDoc, int? privilegeLevel, EliudQuery? eliudQuery }) {
    ...
    Stream<List<DividerModel?>> _values = getQuery(DividerCollection, orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?, limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots().map((snapshot) {
