...
class GridJsFirestore implements GridRepository {
  ...
  Stream<List<GridModel>> values({String orderBy, bool descending }) {
    if (orderBy == null) {
      ...
      return gridCollection.orderBy(orderBy, descending ? 'desc': 'asc').onSnapshot
          .map((data) => data.docs.map((doc) => _populateDoc(doc)).toList());
    }
  }
  ...
  @override
  Future<List<GridModel>> valuesList({String orderBy, bool descending }) {
    if (orderBy == null) {
      ...
      return gridCollection.orderBy(orderBy, descending ? 'desc': 'asc').get().then((value) {
        var list = value.docs;
        return list.map((doc) => _populateDoc(doc)).toList();
      });
    }
