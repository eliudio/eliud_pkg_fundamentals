...
class DecoratedContentFirestore implements DecoratedContentRepository {
  ...
  Future<DecoratedContentModel?> get(String? id, {Function(Exception)? onError}) {
    return DecoratedContentCollection.doc(id).get().then((doc) async {
      if (doc.data() != null)
...
//    In the meantime:
      stream = getQuery(appRepository()!.getSubCollection(appId, 'decoratedcontent'), orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?,  limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots().map((data) {
      Iterable<DecoratedContentModel?> decoratedContents  = data.docs.map((doc) {
        DecoratedContentModel? value = _populateDoc(doc);
        return value;
      }).toList();
      return decoratedContents as List<DecoratedContentModel?>;
    });
...
//  see comment listen(...) above
    stream = getQuery(appRepository()!.getSubCollection(appId, 'decoratedcontent'), orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?,  limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots()
        .asyncMap((data) async {
  ...
  Stream<List<DecoratedContentModel?>> values({String? orderBy, bool? descending, Object? startAfter, int? limit, SetLastDoc? setLastDoc, int? privilegeLevel, EliudQuery? eliudQuery }) {
    ...
    Stream<List<DecoratedContentModel?>> _values = getQuery(DecoratedContentCollection, orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?, limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots().map((snapshot) {
